...
describe('inspector theme tokens', () => {
  it('applyTheme writes field + route tokens for dark and light', () => {
    applyTheme('dark');
    const dark = getComputedStyle(document.documentElement).getPropertyValue('--cs-field-0').trim();
    expect(dark.length).toBeGreaterThan(0);
    applyTheme('light');
    const light = getComputedStyle(document.documentElement).getPropertyValue('--cs-field-0').trim();
