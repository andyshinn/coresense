...
export function FieldCard({ field, scope, hovered, setHovered }: Props) {
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: the card is a focusable hover/focus spotlight target (tabIndex+onFocus/onBlur), not a button — it can't be a <button> because it contains the bits-toggle button.
    // biome-ignore lint/a11y/useAriaPropsSupportedByRole: aria-label announces the field name/range for the focusable spotlight; there's no interactive ARIA role that fits this non-actionable hover/focus target.
    <div
      // biome-ignore lint/a11y/noNoninteractiveTabindex: tabIndex makes the card a focus target so keyboard users get the byte↔field spotlight; the card doesn't perform an action, so no interactive role applies.
      tabIndex={0}
      onMouseEnter={() => setHovered(id)}
      onMouseLeave={() => setHovered(null)}
      onFocus={() => setHovered(id)}
      onBlur={() => setHovered(null)}
      aria-label={`${field.name} field, ${range}`}
