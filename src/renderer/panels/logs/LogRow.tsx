...
export const LogRow: VirtuosoMessageListProps<LogEntry, null>['ItemContent'] = ({ data }) => {
  ...
  return (
    <div className="flex gap-2 px-4 py-0.5 font-mono text-xs hover:bg-cs-bg-3">
      <span className="shrink-0 text-cs-text-dim">{fmtLogTime(data.ts)}</span>
      <span className={`w-14 shrink-0 font-bold uppercase ${LEVEL_COLOR[data.level]}`}>
        {data.level}
      </span>
      <span className="shrink-0 text-cs-text-dim">{source}</span>
      <span className="shrink-0 font-bold text-cs-text">[{data.logger}]</span>
      <span className="min-w-0 flex-1 whitespace-pre-wrap wrap-break-word text-cs-text">
        {data.message}
      </span>
    </div>
  );
