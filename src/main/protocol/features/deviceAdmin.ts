...
// FIFO of in-flight exportPrivateKey() calls awaiting RESP_PRIVATE_KEY /
// RESP_DISABLED. Admin actions are user-initiated and serialise in practice, so
// the oldest entry is always the one the radio is answering.
const pendingExports: PendingExport[] = [];

function removePendingExport(entry: PendingExport): void {
  const i = pendingExports.indexOf(entry);
  if (i !== -1) pendingExports.splice(i, 1);
}

/** Fail every in-flight export awaiter (on disconnect/stop). */
export function resetDeviceAdmin(reason: string): void {
