...
function armDiscovery(prefixHex: string): {
  ...
  const promise = new Promise<DiscoveredPath>((resolve, reject) => {
    const timer = setTimeout(() => {
      if (pendingDiscovery === entry) pendingDiscovery = null;
...
export function resetPathDiagnostics(reason: string): void {
  if (pendingDiscovery) {
    clearTimeout(pendingDiscovery.timer);
    pendingDiscovery.reject(new Error(reason));
    pendingDiscovery = null;
  }
}
...
export const pathDiagnosticsFeature: Feature = {
  ...
  handle: (code, frame) => {
    if (code === PUSH.PATH_UPDATED) {
      ...
      if (pubkeyHex) {
        ...
        if (existing) {
          ...
          log.trace(`path updated: touched ${pubkeyHex.slice(0, 12)}`);
        }
      }
      return;
    }
    ...
    if (!resp) return;
    if (pendingDiscovery && pendingDiscovery.prefixHex === resp.pubKeyPrefixHex) {
      clearTimeout(pendingDiscovery.timer);
      const entry = pendingDiscovery;
      pendingDiscovery = null;
      entry.resolve(resp);
    } else {
      log.debug(`unmatched path discovery response prefix=${resp.pubKeyPrefixHex}`);
    }
  },
};
...
function fullPubKey(contactKey: string, label: string): string {
