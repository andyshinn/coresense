...
// CMD_SET_PATH_HASH_MODE: [0x3d][mode u8]. Radio-wide setting. mode is 0/1/2
// (1/2/4 bytes per hop hash). Persists across reboots on the radio side.
export function buildSetPathHashMode(mode: number): Buffer {
  const m = mode & 0x03;
  return Buffer.from([CMD.SET_PATH_HASH_MODE, m]);
}

/** Convert our per-hop byte size (1|2|4) to the firmware's mode byte (0|1|2). */
export function pathHashSizeToMode(size: 1 | 2 | 4): 0 | 1 | 2 {
  return size === 1 ? 0 : size === 2 ? 1 : 2;
}
/** Inverse of pathHashSizeToMode. */
export function pathHashModeToSize(mode: number): 1 | 2 | 4 {
  return mode === 0 ? 1 : mode === 1 ? 2 : 4;
}
