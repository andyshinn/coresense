...
export class BridgeHub extends EventEmitter {
  ...
  remove(client: BridgeClient): void {
    if (this.clients.delete(client)) {
      ...
      logger.debug(`client removed ${client.kind} ${client.remoteAddr} id=${client.id} (total=${this.clients.size})`);
      this.emit('statusChanged');
    }
  }
  ...
  private enqueueToDevice(client: BridgeClient, payload: Buffer): void {
    if (this.sendQueue.length >= SEND_QUEUE_MAX) {
      logger.warn(`send queue full; dropped ${payload.length}B from ${client.remoteAddr} (queue=${this.sendQueue.length})`);
      emit.error(`Bridge: send queue full; dropped ${payload.length} bytes from ${client.remoteAddr}`);
      return;
    }
    logger.trace(
    ...
    try {
      while (this.sendQueue.length > 0) {
        ...
        if (!transport?.sendBytes) {
          logger.warn(`no radio attached; dropped ${item.payload.length}B from ${item.client.remoteAddr}`);
          emit.error(`Bridge: dropped ${item.payload.length} bytes from ${item.client.remoteAddr} (no radio)`);
          continue;
        }
        try {
          logger.trace(
          ...
          emit.error(`Bridge: send to radio failed for ${item.client.remoteAddr}: ${(err as Error).message}`);
        }
      }
    } finally {
