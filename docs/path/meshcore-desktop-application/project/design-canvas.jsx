  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        ...
        apply();
        // If the saved pan now points to empty space (e.g. content above was
        // deleted between sessions) the world's bbox won't intersect the
        // viewport. Reset to identity so the user lands on something.
        requestAnimationFrame(() => {
          const vp = vpRef.current, world = worldRef.current;
          if (!vp || !world) return;
          const vr = vp.getBoundingClientRect();
          const wr = world.getBoundingClientRect();
          const intersects = wr.right > vr.left && wr.left < vr.right
                          && wr.bottom > vr.top && wr.top < vr.bottom;
          if (!intersects) {
